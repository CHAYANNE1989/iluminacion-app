...
if st.session_state.proyectos:
    ...
    if proyecto_data["planos"]:
        ...
        for i, (x, y) in enumerate(plano_data["puntos"]):
            ...
            foto_subida = st.file_uploader(f"Foto punto {i+1} (opcional)", type=["jpg", "jpeg", "png"], key=f"foto_{i}")
...
if st.session_state.proyectos:
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button(f"Descargar PDF SOLO de '{st.session_state.proyecto_actual}'", key="pdf_individual_global"):
