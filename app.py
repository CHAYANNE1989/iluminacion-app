...
ARLS = ["Positiva","Colmena","Bolívar","AXA Colpatria","Sura"]
PLANTILLAS_ARL = {
    ...
    for idx,(pnombre,pdata) in enumerate(st.session_state.proyectos.items()):
        ...
        with st.container(border=True):
            ...
            with ci:
                ...
                st.caption(f"📋 OT: {g.get('numero_orden','N/A')}  |  📍 {g.get('sede','N/A')}  |  📅 {g.get('fecha','N/A')}  |  🏥 ARL: {g.get('arl','N/A')}")
                if tot>0:
                    ...
                    with st.spinner("Generando Word..."):
                        try:
                            ...
                            word_buf=generar_informe_word(g,todas_med,plano_imgs,
                                arl=g.get("arl","Positiva"),
                                plantillas_arl=PLANTILLAS_ARL)
...
def pagina_nuevo_proyecto():
    ...
    with st.form("form_nuevo"):
        ...
        st.subheader("🏥 ARL")
        arl=st.selectbox("ARL que patrocina el estudio *", ARLS, key="sel_arl_new")
        st.subheader("🔬 Equipo de Medición")
        ce1,ce2,ce3=st.columns(3)
        with ce1: eq_marca =st.text_input("Marca del luxómetro",  value="Hanger")
        with ce2: eq_modelo=st.text_input("Modelo",               value="EC1")
        with ce3: eq_serie =st.text_input("N° de Serie",          value="54815")
            ...
            if pnombre in st.session_state.proyectos: st.error("❌ Ya existe")
                ...
                st.session_state.proyectos[pnombre]={
                    "general":{"numero_orden":num_orden,"nombre_empresa":nom_emp,"nit":nit,
                               ...
                               "responsable_empresa":resp_e,"responsable_higienista":resp_h,
                               "resolucion":resol,"arl":arl,
                               "equipo":{"instrumento":"Luxómetro","marca":eq_marca,
                                         "modelo":eq_modelo,"serie":eq_serie}},
                    "planos":{}}
    ...
    if st.session_state.get("_show_edit",False):
        with st.expander("📝 Editar información",expanded=True):
            with st.form("form_edit_gral"):
                ...
                arl_act=g.get("arl","Positiva")
                v_arl=st.selectbox("ARL",ARLS,
                    index=ARLS.index(arl_act) if arl_act in ARLS else 0,
                    key="sel_arl_edit")
                eq=g.get("equipo",{})
                cee1,cee2,cee3=st.columns(3)
                with cee1: v_marca =st.text_input("Marca luxómetro",value=eq.get("marca","Hanger"))
                with cee2: v_modelo=st.text_input("Modelo",         value=eq.get("modelo","EC1"))
                with cee3: v_serie =st.text_input("N° de Serie",    value=eq.get("serie","54815"))
                if st.form_submit_button("💾 Guardar cambios",use_container_width=True):
                    g.update({"numero_orden":v_or,"nombre_empresa":v_em,"nit":v_ni,"direccion":v_di,
                              ...
                              "responsable_higienista":v_hi,"resolucion":v_rs,"arl":v_arl,
                              "equipo":{"instrumento":"Luxómetro","marca":v_marca,
                                        "modelo":v_modelo,"serie":v_serie}})
                    guardar_proyectos(st.session_state.proyectos)
